...
using CemeteryManagementSystem.Models.TokenAuth;
...
public abstract class CemeteryManagementSystemWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
{
    protected static readonly Lazy<string> ContentRootFolder;

    static CemeteryManagementSystemWebTestBase()
    {
    ...
    protected override IWebHostBuilder CreateWebHostBuilder()
    {
        return base
            ...
            .UseSetting(WebHostDefaults.ApplicationKey, typeof(CemeteryManagementSystemWebMvcModule).Assembly.FullName);
    }
    ...
    {
        using (var context = IocManager.Resolve<CemeteryManagementSystemDbContext>())
        {
            action(context);
            context.SaveChanges();
        }
    ...
    protected async Task UsingDbContextAsync(Func<CemeteryManagementSystemDbContext, Task> action)
    {
        using (var context = IocManager.Resolve<CemeteryManagementSystemDbContext>())
        {
    ...
    protected async Task<T> UsingDbContextAsync<T>(Func<CemeteryManagementSystemDbContext, Task<T>> func)
    {
        T result;
